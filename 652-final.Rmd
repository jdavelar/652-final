...
*Note*: I will use VV for the comments I make so you could simly find "VV" for my comments in this document
...
# VV - will using mutate and drop_na from tidyverse help perhaps?
# geo_code[c(1:11), 1] = 1
...
ltviz <-  enroll %>% 
  dplyr::select(srvy_year, language, undergrad_total, grad_total) %>%  #VV -specifying dplyr for select function is especially useful because the order in which you load your libraries affects if select is overwritten from a function in another package. I have found this helpful
...
# Set up labels for ranking while you're at it
  ...
  mutate(rank = rank(-total),
         value_rel = total/total[rank==1], #VV- this is very quick and useful, wonder if you could also achieve thins by using asc or sort/order
...
animate(anim, nframes = 350, fps = 25, width = 1200, height = 1000,
        renderer = gifski_renderer("viz1.gif")) #VV - Very cool!!
...
# tibble I can map using total state means. The work up to this point is to have a datset
...
#VV = This has definitely been quite a journey for you. I would recommend using choreopath or a package like leaflet - this is a very helpful package more so if you are specifically looking at US data. Also, having a legend would help to indicate what the changing colors indicate. May be, labels for the states?
...
#VV =  This is a very cool plot. It would really help to may be annotate it and work with the axes labels. Also it would be very helpful to explain what exactly is the plot displaying and what do the changing jitter animations refer to. 
```
...
- At least three areas of strength
  - Extensive playing around with the themes for ggplot
  - Using gganimate for the first plot is really helpful
  - The map looks great in the sense of integrating the lat long data and even getting here is an accomplishment for sure
- At least one thing you learned from reviewing their script
  - The use of the rank function is very helpful, its simple but I think I found a solution for one of my problems.
  - gganimate package - I was feeling a little sceptical about this package but after seeing your code, I feel a little more confident with using it in my furture work perhaps
- at least one and no more than three areas for improvement for each visualization.
I have mentioned comments in the code sectiions as I went along. [find "VV" for the comments]
- Comments on both the code leading up to (and including) the visualization, and the visualization itself (aesthetics, best practices, etc.).
   - The comments you keep making are very helpful, especially catered towards someone who is working on a project individually, but it definitely helps to navigate. 
   - I completely agree with the writing of issues that you note for yourself, they are super handy!
   - Suggestion - It would be helpful to also make a note of 'why' exactly are you doing at each step with the variables be it selecting or filtering alongwith 'what' you are doing. I am sure this is not possible every time or not even required but when you are using 'distinct()' or 'unique()', it will be helpful to know why is that necessary. As I mention this here, I am also making a note for myself as I am still learning to make sure I am doing this. 
   - Thanks for providing some of the NYT and 'how to do' this links, I am going to save them. 
   - I think the colors you have used really work well, not sure if they are colorblind though
  - The most important learning I am taking away is that using gganimate is not that difficult and its important to ungroup() sometimes after group_by()
  - Awesome job!
