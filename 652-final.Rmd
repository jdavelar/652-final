...
library(sf)
library(maps) #needed
library(mapproj) #needed

# Map data for literal map
states <- map_data("state")


# Trying this alternate route b/c HI and AK???
# install.packages("usmap")
# library(usmap)
# states <- plot_usmap(regions = "states")
# str(usmap::us_map(regions = "states"))
...
# Join and find state prop
## Back up!!! You need individual unitid data to map
# prop = average enrollment/institution
# map showing highest concentrations of enrollment per state by mean students enrolled in language courses? total lang enrolled/total enrolled and map points on top of that with institution level data if able to make it interactive (name/year, students per language for top 5 or something)
...
# This is beautiful, and now I'm going to mess it all up by butchering it into a 50 row tibble I can map using total state means. The work up to this point is to have a datset ready with the information I need for the plot below, and also with institution names, lats, and longs to add points to the map later if I'm able to.
...
# left-join map data for plotting
...
viz2_map %>% 
  ...
  geom_map(aes(map_id = region),
           ...
           color = "gray80", fill = "gray30", size = 0.3) +
  coord_map("ortho", orientation = c(39, -98, 0)) +
  geom_polygon(aes(group = group, fill = state_lang_prop) +
